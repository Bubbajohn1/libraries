--[[
    ...
    fixed by desktop2677 semi loves my big cock
...
-- // Tables
local Library, Utility, Flags, Theme = loadstring(game:HttpGet("https://gist.githubusercontent.com/loader-live/1cdba58dc682c9ac2848ae2a6575c9b0/raw/0348b1d0d48350c861728124ed21f48fa6d0ccb4/gistfile1.txt"))()
--
...
do -- Atlanta
    ...
    function Atlanta:GetTriggerBotTarget()
        ...
        if Hit then
            Atlanta.Locals.TriggerTarget = {
                Part = Hit,
                Position = Position, -- autistic dildo desktop2677
                Material = Material
            }
        else
    ...
    function Atlanta:AimAssist()
        if Atlanta.Locals.Target and Atlanta.Locals.Target.Part and Atlanta.Locals.Target.Vector then
            ...
            end-- autistic dildo desktop2677
        end
    end
    --
    ...
    do -- Game LUAs
                    ...
                    if typeof(Value) == "function" and islclosure(Value) and not is_synapse_function(Value) then
                        for Index, Value in pairs(getupvalues(Value)) do
                            if type(Value) == "table" and rawget(Value, "Kitty") then
                                Modules = getupvalue(getrawmetatable(Value).__index, 1);break -- autistic dildo desktop2677
                            end
                        end
                    end
                end
        ...
        for Index, Value in pairs(Games) do
            if Find(Index, PlaceId) then
                Utility:ThreadFunction(Value.Func, "Lx01") -- autistic dildo desktop2677
            end
        end
    end
end
